...
def rbf(x_1, x_2, sigma=1.):
    ...
    n_obj_1 = x_1_np.shape[0]
    n_obj_2 = x_2_np.shape[0]
    n_features = x_1_np.shape[1]

    mults = x_1_np @ (x_2_np.T)

    v_norms_1 = np.sum(x_1_np**2, axis=1)
    martix_norms_1 = np.zeros(n_obj_1, n_obj_2) + v_norms_1

    v_norms_2 = np.sum(x_2_np**2, axis=1)
    martix_norms_2 = np.zeros(n_obj_2, n_obj_1) + v_norms_2
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, n_obj, self.batch_size):
                ...
                preds = self.predict(x_batch)### YOUR CODE HERE # get the matrix product using SVM parameters: self.betas and self.bias
                preds = preds.flatten()
    ...
    def predict_scores(self, batch):
        with torch.no_grad():
            ...
            return torch.sum(self.betas.unsqueeze(0).expand(len(batch), -1) * K, axis=1) + self.bias
